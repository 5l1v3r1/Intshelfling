...
def process_data(value,id):
    ...
    prt = "Liczba puszek na " + str(id) + " segmencie: " + str(data)
    print (prt)
    ...
    values[id]=values[id].rpartition(':')[2]
    ile=int(values[id])-data
    
    
    
    
    if(ile>0):
        updating(id,0,int(ile))
    elif(ile<0):
...
while True:
    try:
        ...
        while licznik<4:
            ...
            time.sleep(0.1)
            licznik+=1
        process_data(median(val),0)
        process_data(median(val2),1)
